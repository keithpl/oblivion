...
#define list_for_each_prev_safe(head, pos, tmp)				\
...
#define list_for_each_entry_prev(head, pos, type, field)		\
...
#define list_for_each_entry_prev_safe(head, pos, tmp, type, field)	\
	for (pos = list_last(head, type, field),			\
...
{
	list_link(prev, entry);
	list_link(entry, next);
}
...
{
	list_link(entry->prev, entry->next);
}
